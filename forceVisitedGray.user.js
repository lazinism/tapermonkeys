// ==UserScript==
// @name         네이버 카페 방문 페이지 회색표시하기 (URL 구조 개선)
...
(function () {
    ...
    function getClubAndArticleIdFromUrl(url) {
        ...
        let clubid, articleid;

        // 1. 기존 query 방식 (예: ?clubid=1234&articleid=5678)
        clubid = urlObj.searchParams.get('clubid');
        articleid = urlObj.searchParams.get('articleid');
        const commentFocus = urlObj.searchParams.get('commentFocus');

        // 2. 새로운 pathname 방식 (예: /f-e/cafes/1234/articles/5678)
        if (!clubid || !articleid) {
            const pathParts = urlObj.pathname.split('/');
            const clubIndex = pathParts.indexOf('cafes');
            const articleIndex = pathParts.indexOf('articles');
            if (clubIndex !== -1 && articleIndex !== -1 && articleIndex > clubIndex) {
                clubid = pathParts[clubIndex + 1];
    ...
    function updateVisitedLinkStyles() {
        ...
        const links = document.querySelectorAll('a[href*="articleid="], a[href*="/articles/"]');
